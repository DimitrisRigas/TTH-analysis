...
void MyClass::Loop() {
 ...
 // H->bb backgrounds: multiply by BR(H->bb)
  ...
  TH1F *h_dR_jet_ele_before = new TH1F("h_dR_jet_ele_before", "#Delta R(jet, electron) BEFORE cleaning; #Delta R; Entries", 100, 0, 5);
  TH1F *h_dR_jet_mu_before  = new TH1F("h_dR_jet_mu_before", "#Delta R(jet, muon) BEFORE cleaning; #Delta R; Entries", 100, 0, 5);
  TH1F *h_dR_jet_ele_after  = new TH1F("h_dR_jet_ele_after", "#Delta R(jet, electron) AFTER cleaning; #Delta R; Entries", 100, 0, 5);
  TH1F *h_dR_jet_mu_after   = new TH1F("h_dR_jet_mu_after", "#Delta R(jet, muon) AFTER cleaning; #Delta R; Entries", 100, 0, 5);

  // --- Reconstructed Higgs kinematics ---
  TH1F *h_Hdbb_pt  = new TH1F("h_Hdbb_pt", "Reconstructed Higgs p_{T}; p_{T} [GeV]; Entries", 100, 0, 1000);
  TH1F *h_Hdbb_eta = new TH1F("h_Hdbb_eta", "Reconstructed Higgs #eta; #eta; Entries", 60, -5, 5);

  // --- Double-b jet kinematics (leading + subleading) ---
  TH1F *h_dbj1_pt  = new TH1F("h_dbj1_pt", "Double-b jet 1 p_{T}; p_{T} [GeV]; Entries", 100, 0, 500);
  TH1F *h_dbj1_eta = new TH1F("h_dbj1_eta", "Double-b jet 1 #eta; #eta; Entries", 60, -2.5, 2.5);
  TH1F *h_dbj2_pt  = new TH1F("h_dbj2_pt", "Double-b jet 2 p_{T}; p_{T} [GeV]; Entries", 100, 0, 500);
  TH1F *h_dbj2_eta = new TH1F("h_dbj2_eta", "Double-b jet 2 #eta; #eta; Entries", 60, -2.5, 2.5);
 ...
 TH1F *h_e_pt_rank [MAXELE];
  ...
  TH1F *h_Hdbb_mass     = new TH1F("h_Hdbb_mass", "Reconstructed Higgs mass from double-b jets; m_{bb} [GeV]; Entries", 80, 0, 500);
  TH1F *h_MET_pt_final  = new TH1F("h_MET_pt_final", "MET (final selection); MET [GeV]; Entries", 100, 0, 500);
  TH1F *h_MET_phi_final = new TH1F("h_MET_phi_final", "MET #phi (final selection); #phi; Entries", 64, -3.2, 3.2);
  TH1F *h_dbj1_mass     = new TH1F("h_dbj1_mass", "Double-b jet 1 mass; m [GeV]; Entries", 80, 0, 80);
  TH1F *h_dbj2_mass     = new TH1F("h_dbj2_mass", "Double-b jet 2 mass; m [GeV]; Entries", 80, 0, 80);
  TH1F *h_HT            = new TH1F("h_HT", "H_{T} scalar sum of jet p_{T}; H_{T} [GeV]; Entries", 100, 0, 2000);

  // ==========================================================
  // NEW: FINAL RECO B-JET KINEMATICS (after full selection)
  // ==========================================================
  TH1F *h_bj1_pt_final  = new TH1F( "h_bj1_pt_final", "Leading b-jet p_{T} (final); p_{T} [GeV]; Entries", 100, 0, 500 );
  TH1F *h_bj1_eta_final = new TH1F( "h_bj1_eta_final", "Leading b-jet #eta (final); #eta; Entries", 60, -2.5, 2.5 );
  TH1F *h_bj2_pt_final  = new TH1F( "h_bj2_pt_final", "Subleading b-jet p_{T} (final); p_{T} [GeV]; Entries", 100, 0, 500 );
  ...
  // ==========================================================
  TH1F *h_dR_dbj12 = new TH1F( "h_dR_dbj12_final", "#Delta R(double-b jet 1, double-b jet 2) (final); #Delta R; Entries", 100, 0, 5 );
  TH1F *h_dbj1_phi = new TH1F( "h_dbj1_phi_final", "Double-b jet 1 #phi (final); #phi; Entries", 64, -3.2, 3.2 );
  TH1F *h_dbj2_phi = new TH1F( "h_dbj2_phi_final", "Double-b jet 2 #phi (final); #phi; Entries", 64, -3.2, 3.2 );
  TH1F *h_lep1_pt  = new TH1F( "h_lep1_pt_final", "Leading lepton p_{T} (final); p_{T} [GeV]; Entries", 100, 0, 500 );
  TH1F *h_lep1_eta = new TH1F( "h_lep1_eta_final", "Leading lepton #eta (final); #eta; Entries", 60, -2.5, 2.5 );
  TH1F *h_lep1_phi = new TH1F( "h_lep1_phi_final", "Leading lepton #phi (final); #phi; Entries", 64, -3.2, 3.2 );
  TH1F *h_lep2_pt  = new TH1F( "h_lep2_pt_final", "Subleading lepton p_{T} (final); p_{T} [GeV]; Entries", 100, 0, 500 );
  TH1F *h_lep2_eta = new TH1F( "h_lep2_eta_final", "Subleading lepton #eta (final); #eta; Entries", 60, -2.5, 2.5 );
    ...
    if (isSignal) {
      ...
      for (int i = 0; i < nGenPart; ++i) {
        ...
        for (int j = 0; j < nGenPart; ++j) {
          if (GenPart_genPartIdxMother[j] == i && std::abs(GenPart_pdgId[j]) == 25) {
      ...
      for (int ih : final_higgs) {
        ...
        for (int j = 0; j < nGenPart; ++j) {
          if (GenPart_genPartIdxMother[j] == ih && std::abs(GenPart_pdgId[j]) == 36) {
        ...
        if (!a_indices.empty()) {
          ...
          for (int k = 0; k < nGenPart; ++k) {
            if (GenPart_genPartIdxMother[k] == ia1 && std::abs(GenPart_pdgId[k]) == 5) {
        ...
        if (a_indices.size() > 1) {
          ...
          for (int k = 0; k < nGenPart; ++k) {
            if (GenPart_genPartIdxMother[k] == ia2 && std::abs(GenPart_pdgId[k]) == 5) {
        ...
        for (int ia : a_indices) {
          for (int k = 0; k < nGenPart; ++k) {
            if (GenPart_genPartIdxMother[k] == ia && std::abs(GenPart_pdgId[k]) == 5) {
      ...
      auto processTop = [&](int idx, int rank) {
        ...
        while (search) {
          search = false;
          for (int j = 0; j < nGenPart; ++j) {
            if (GenPart_genPartIdxMother[j] == final_top && std::abs(GenPart_pdgId[j]) == 6) {
              final_top = j;
              search = true;
              break;
            }
        ...
        if (w_idx != -1) {
          ...
          while (!isFinal) {
            ...
            for (int k = 0; k < nGenPart; ++k) {
              if (GenPart_genPartIdxMother[k] == final_w && std::abs(GenPart_pdgId[k]) == 24) {
                final_w = k;
                hasDaughter = true;
                break;
              }
          ...
          for (int k = 0; k < nGenPart; ++k) {
            ...
            if (absId >= 11 && absId <= 16) {
              isLep = true;
              if (absId % 2 == 1) h_lep_from_W_pt->Fill(GenPart_pt[k]);
              else                h_nu_from_W_pt ->Fill(GenPart_pt[k]);
            }
          }
...
// ======================= PART 2/3 =======================
    ...
    for (int i = 0; i < nJet; ++i) {
      ...
      // plot double-b tagger:
      // h_doubleb->Fill(Jet_btagUParTAK3probbb[i]);
      // Plot b-tagger
      // h_singleb->Fill();
