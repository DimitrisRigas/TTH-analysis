...
// Derived fit info
// ============================================================
struct DerivedBkgFit {
  double Nbkg = 0.0;
  double f1   = 0.0;   // floating tt fraction
  double f2   = 0.0;   // fixed ttsemi nominal fraction

  double val_tt = 0.0;
  double val_ttsemi = 0.0;

  double err_tt = 0.0;
  double err_ttsemi = 0.0;

  double corr_tt_ttsemi = 0.0;
  double corr_Nbkg_f1   = 0.0;
};

static DerivedBkgFit computeDerivedBkgFit(const RooFitResult* fitres,
                                          const RooRealVar* Nexp_bkg,
                                          const RooRealVar* f1,
                                          const RooConstVar* f2) {
  DerivedBkgFit d;

  if (!Nexp_bkg || !f1) return d;

  d.Nbkg = Nexp_bkg->getVal();
  d.f1   = f1->getVal();
  d.f2   = (f2 ? f2->getVal() : (1.0 - d.f1));

  const double sN = Nexp_bkg->getError();
  const double sf = f1->getError();

  double rho = 0.0;
  if (fitres) {
    rho = fitres->correlation(*Nexp_bkg, *f1);
  }
  d.corr_Nbkg_f1 = rho;

  const double VNN = sN * sN;
  const double Vff = sf * sf;
  const double VNf = rho * sN * sf;

  const double N   = d.Nbkg;
  const double f   = d.f1;
  ...
  const double var_tt =
    f * f * VNN + N * N * Vff + 2.0 * N * f * VNf;

  const double var_ttsemi =
    omf * omf * VNN + N * N * Vff - 2.0 * N * omf * VNf;

  const double cov_tt_ttsemi =
    f * omf * VNN + N * (1.0 - 2.0 * f) * VNf - N * N * Vff;

  d.err_tt     = std::sqrt(std::max(0.0, var_tt));
  d.err_ttsemi = std::sqrt(std::max(0.0, var_ttsemi));

  const double denom = d.err_tt * d.err_ttsemi;
  d.corr_tt_ttsemi = (denom > 0.0 ? cov_tt_ttsemi / denom : 0.0);
...
struct AnalysisModel {
  ...
  RooAddPdf*  bkg_pdf    = nullptr;

  RooRealVar* Nexp_sig = nullptr;
  RooRealVar* Nexp_bkg = nullptr;

  RooRealVar*  f1 = nullptr;   // floating tt fraction
  RooConstVar* f2 = nullptr;   // fixed ttsemi nominal fraction (bookkeeping only)
  ...
  RooAbsPdf* model_0 = nullptr;
  RooAbsPdf* model_1 = nullptr;
...
bool constructModel(const Histograms& hs, AnalysisModel& fm) {
  ...
  const double f1_nom = (hs.Nbkg > 0.0 ? hs.Ntt     / hs.Nbkg : 0.5);
  const double f2_nom = (hs.Nbkg > 0.0 ? hs.Nttsemi / hs.Nbkg : 0.5);

  fm.Nexp_sig = new RooRealVar("Nexp_sig", "Expected signal events",
                               hs.Nsig, 0.0, 1000.0 * std::max(1.0, hs.Nsig));

  fm.Nexp_bkg = new RooRealVar("Nexp_bkg", "Expected total background",
                               hs.Nbkg, 0.0, 10.0 * std::max(1.0, hs.Nbkg));

  fm.f1 = new RooRealVar("f1", "fraction of tt", f1_nom, 0.0, 1.0);
  fm.f2 = new RooConstVar("f2", "fixed fraction of ttsemi", f2_nom);

  fm.bkg_pdf = new RooAddPdf("bkg_pdf", "bkg_pdf",
                             RooArgList(*fm.pdf_tt, *fm.pdf_ttsemi),
                             RooArgList(*fm.f1));

  fm.model_0 = new RooExtendPdf("model_0", "Background-only extended model",
                                *fm.bkg_pdf, *fm.Nexp_bkg);

  fm.model_1 = new RooAddPdf("model_1", "Signal+Background (extended)",
                             RooArgList(*fm.pdf_sig, *fm.bkg_pdf),
...
// ============================================================
// Construct RooFit model (background only)
// ============================================================
...
void printOneFitSummary(int mass, const Histograms& hs, AnalysisModel& fm, RooFitResult* fitres) {
  ...
  DerivedBkgFit d = computeDerivedBkgFit(fitres, fm.Nexp_bkg, fm.f1, fm.f2);
  ...
  std::cout << "\n--- Floating / fixed fractions ---\n";
  std::cout << "Nexp_bkg = " << fm.Nexp_bkg->getVal()
            << " +/- " << fm.Nexp_bkg->getError() << "\n";
  std::cout << "f1 (float) = " << fm.f1->getVal()
            << " +/- " << fm.f1->getError() << "\n";
  std::cout << "f2 (const) = " << fm.f2->getVal() << "\n";

  std::cout << "\n--- Derived component yields ---\n";
  std::cout << "Nexp_tt(fit)     = " << d.val_tt     << " +/- " << d.err_tt     << "\n";
  std::cout << "Nexp_ttsemi(fit) = " << d.val_ttsemi << " +/- " << d.err_ttsemi << "\n";
  ...
  const double pull_f1 =
    (fm.f1->getError() > 0.0) ? ((hs.Nbkg > 0.0 ? hs.Ntt / hs.Nbkg : 0.0) - fm.f1->getVal()) / fm.f1->getError() : 0.0;

  const double pull_tt =
    (d.err_tt > 0.0) ? (hs.Ntt - d.val_tt) / d.err_tt : 0.0;
  const double pull_ttsemi =
    (d.err_ttsemi > 0.0) ? (hs.Nttsemi - d.val_ttsemi) / d.err_ttsemi : 0.0;

  std::cout << "\n--- Pulls ---\n";
  std::cout << "pull(f1)             = " << pull_f1 << "\n";
  std::cout << "pull(TTbar yield)    = " << pull_tt << "\n";
  std::cout << "pull(TTsemi yield)   = " << pull_ttsemi << "\n";
  std::cout << "pull(f2)             = not defined (f2 is constant)\n";

  std::cout << "\n--- Correlations ---\n";
  std::cout << "corr(Nexp_bkg, f1)         = " << d.corr_Nbkg_f1 << "\n";
  std::cout << "corr(Nexp_tt, Nexp_ttsemi) = " << d.corr_tt_ttsemi << "\n";
...
void plotOneToyFit(int mass, AnalysisModel& fm, RooDataHist& toyData) {
  ...
  RooPlot* fr = fm.output_BDT->frame(Binning(*fm.customBinning));

  toyData.plotOn(fr, Name("toydata"), Binning(*fm.customBinning));
...
// ============================================================
// Toy MC study for pulls
// ============================================================
void runToyPullStudy(const Histograms& hs, AnalysisModel& fm) {
  TH1F* h_pullDist_f1 = new TH1F(
    "h_pullDist_f1_bonly",
    "f1 pull distribution;Pull;Toys",
    60, -5, 5
  );

  TH1F* h_pullDist_tt = new TH1F(
    "h_pullDist_tt_bonly",
    "TTbar yield pull distribution;Pull;Toys",
    60, -5, 5
  );

  TH1F* h_pullDist_ttsemi = new TH1F(
    "h_pullDist_ttsemi_bonly",
    "TT semileptonic yield pull distribution;Pull;Toys",
    60, -5, 5
  );

  TH1F* h_fitval_f1 = new TH1F(
    "h_fitval_f1_bonly",
    "Fitted f1;f1;Toys",
    60, 0.0, 1.0
  );

  TH1F* h_fiterr_f1 = new TH1F(
    "h_fiterr_f1_bonly",
    "Fit error on f1;#sigma(f1);Toys",
    60, 0.0, 1.0
  );
  ...
  const double f1_nominal   = (hs.Nbkg > 0.0 ? hs.Ntt / hs.Nbkg : 0.5);
  const double f2_nominal   = (hs.Nbkg > 0.0 ? hs.Nttsemi / hs.Nbkg : 0.5);
  ...
  for (int it = 0; it < opt.Ntoys_pulls; ++it) {
    ...
    const double f1_err = fm.f1->getError();
    const double pull_f1 =
      (f1_err > 0.0) ? (f1_nominal - fm.f1->getVal()) / f1_err : 999.0;

    const double pull_tt =
      (d.err_tt > 0.0) ? (hs.Ntt - d.val_tt) / d.err_tt : 999.0;

    const double pull_ttsemi =
      (d.err_ttsemi > 0.0) ? (hs.Nttsemi - d.val_ttsemi) / d.err_ttsemi : 999.0;

    if (std::fabs(pull_f1) > 3.0 || std::fabs(pull_tt) > 3.0 || std::fabs(pull_ttsemi) > 3.0) {
      std::cout << "\n[TAIL TOY] it = " << it
                ...
                << "  pull = " << pull_ttsemi
                << "\n"
                << "  corr(Nexp_bkg, f1) = " << d.corr_Nbkg_f1
                << "\n"
                << "  corr(Nexp_tt, Nexp_ttsemi) = " << d.corr_tt_ttsemi
                << "\n";
    }

    h_fitval_f1->Fill(fm.f1->getVal());
    h_fiterr_f1->Fill(f1_err);

    h_fitval_tt->Fill(d.val_tt);
    h_fitval_ttsemi->Fill(d.val_ttsemi);
    h_err_tt->Fill(d.err_tt);
    h_err_ttsemi->Fill(d.err_ttsemi);

    if (f1_err > 0.0) {
      h_pullDist_f1->Fill(pull_f1);
    }
    if (d.err_tt > 0.0) {
      h_pullDist_tt->Fill(pull_tt);
    }
    if (d.err_ttsemi > 0.0) {
      h_pullDist_ttsemi->Fill(pull_ttsemi);
    }
  ...
  std::cout << "f1 pull:       mean = " << h_pullDist_f1->GetMean()
            << "   RMS = " << h_pullDist_f1->GetRMS() << "\n";
  ...
  std::cout << "f2 pull: not defined because f2 is constant\n";

  {
    TCanvas* c0 = new TCanvas("c_pullDist_f1_bonly", "f1 pull", 800, 600);
    h_pullDist_f1->Fit("gaus", "Q", "");
    h_pullDist_f1->Draw();
    c0->SaveAs("fits_and_limit_plots/step4_pull_distribution_f1_bonly.pdf");
    delete c0;
  }
  ...
  {
    TCanvas* c2b = new TCanvas("c_fitval_f1_bonly", "Fitted f1", 800, 600);
    h_fitval_f1->Draw();
    c2b->SaveAs("fits_and_limit_plots/step4_fitval_f1_bonly.pdf");
    delete c2b;
  }

  {
    TCanvas* c2c = new TCanvas("c_fiterr_f1_bonly", "Error f1", 800, 600);
    h_fiterr_f1->Draw();
    c2c->SaveAs("fits_and_limit_plots/step4_fiterr_f1_bonly.pdf");
    delete c2c;
  }
...
// ============================================================
// Build workspace and proposal for limits
// ============================================================
void buildWorkspaceAndProposal(AnalysisModel& fm, RooDataHist& data_asimov_B) {
  ...
  RooArgSet nuis(*fm.Nexp_bkg, *fm.f1);
